...
class HTB:
    ...
    def router_command(self, router, interface):
        ...
        router.cmd(f'tc class add dev {interface} parent 1: classid 1:1 htb rate 500mbit')
        router.cmd(f'tc class add dev {interface} parent 1: classid 1:2 htb rate 500mbit')
        router.cmd(f'tc class add dev {interface} parent 1: classid 1:10 htb rate 100mbit')
