...
def udpConnect(h, port):
    port_no = port - 1
    h.cmd('iperf3 -c 10.0.6.10 -u -b 1000g -p 520%i -J > h%i_udp_result.json' % (port_no,port))

def tcpConnect(h, port):
    port_no = port - 1
    h.cmd('iperf3 -c 10.0.6.10 -b 1g -p 520%i -J > h%i_tcp_result.json' % (port_no,port))
...
def main():
    ...
    # r.cmd('tc qdisc add dev r-eth5 root %s' % Queue_disc)
    ##==============================================================================
    ## Classful AQM
